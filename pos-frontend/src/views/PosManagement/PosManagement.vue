<template>
  <AdminLayout>
        ...
        <div class="flex">
          ...
          <div class="flex flex-wrap gap-2">
            <button v-for="category in categories" :key="category"
                    ...
                    class="px-3 py-2 rounded text-sm font-medium transition-colors" @click="activeCategory = category">
              {{ category }}
            </button>
          </div>
        </div>
      ...
      <div
        ...
        <div class="flex gap-2">
          <div v-for="tab in tabs" :key="tab.id"
            ...
            <!-- Nút chọn tab -->
            <button class="px-3 py-1 text-sm font-medium flex-1 text-left"
                    @click="activeTabId = tab.id">
              {{ tab.name }}
            </button>
            <!-- Icon xoá tab -->
            <button class="px-1 flex items-center justify-center text-xs hover:text-red-600 transition"
          ...
          <button class="px-3 py-1 rounded bg-green-500 text-white" @click="addNewTab">+</button>
        </div>
        ...
        <div class="flex-1 overflow-y-auto max-h-[450px] flex flex-col gap-4 pr-1 custom-scrollbar">
          ...
          <CartDetailListing :orders="activeTab.orders" @clear="clearOrders"
                             @decrease="decreaseQuantity" @increase="increaseQuantity"/>
        </div>
        ...
        <div class="border-t pt-2 space-y-1 text-sm border-gray-200 dark:border-gray-700">
          <div class="flex justify-between">
            ...
            <span
              class="text-gray-800 dark:text-gray-200">{{ subTotal.toLocaleString() }} VNĐ</span>
          </div>
          <div class="flex justify-between">
        ...
        <div class="sticky bottom-0 bg-white dark:bg-gray-900 flex gap-2">
          ...
          <button class="inline-flex items-center justify-center font-medium gap-2 rounded-lg transition w-10 aspect-square bg-gray-300 dark:bg-gray-700 text-black dark:text-gray-200"
    ...
    <InvoiceActionsModal v-if="showActionModal" :invoice="activeTab"
                         @cancel="cancelInvoice"
                         @close="showActionModal = false" @print="printInvoice"
                         @split="splitInvoice" @change-status="changeInvoiceStatus"
...
export default defineComponent({
  ...
  computed: {
    ...
    subTotal() {
      return this.activeTab.orders.reduce((sum, order) => sum + order.price * order.quantity, 0)
    },
    tax() {
