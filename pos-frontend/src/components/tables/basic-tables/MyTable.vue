<template>
  <div class="overflow-hidden bg-white dark:border-gray-800 dark:bg-white/[0.03]">
      ...
      <table class="min-w-full table-auto text-sm text-gray-700 dark:text-gray-300">
        ...
        <tr v-for="(row, rowIndex) in paginatedRows" :key="rowIndex"
          ...
          <td v-for="(column, colIndex) in columns" :key="colIndex"
            ...
            <slot :column="column" :name="`cell-${column.key}`" :row="row">
              <span class="text-gray-700 dark:text-gray-300">{{ row[column.key] }}</span>
            </slot>
          </td>
          <td class="px-4 py-3 sm:px-6 whitespace-nowrap">
            <div class="flex space-x-2">
                                <span class="inline-block px-2 py-1 rounded text-xs font-semibold cursor-pointer bg-green-100 text-black hover:bg-green-200 dark:bg-green-200/10 dark:text-green-400 transition"
                                      @click="showModal(row)">
                                    Xem / Sửa
                                </span>
            </div>
    ...
    <!-- Pagination -->
    <Pagination :current-page="currentPage" :total-pages="totalPages" @next="nextPage"
                @prev="prevPage"/>

    <Transition name="fade-modal">
      <EditModal v-if="popupDetail" :columns="columns" :header-description="modalHeaderDescription"
                 :header-title="modalHeaderTitle"
                 :row="selectedRow" @close="popupDetail = false"
                 @save="saveProfile"/>
    </Transition>
  </div>
</template>
