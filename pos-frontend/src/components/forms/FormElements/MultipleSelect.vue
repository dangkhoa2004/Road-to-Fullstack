<template>
  <div ref="multiSelectRef" class="relative">
    <div :class="{ 'text-gray-800 dark:text-white/90': isOpen }"
      ...
      <div class="flex flex-wrap items-center flex-auto gap-2">
        <div v-for="item in selectedItems" :key="item.value"
          ...
          <button aria-label="Remove item"
            ...
            <svg fill="none" height="14" role="button" viewBox="0 0 14 14" width="14"
                 xmlns="http://www.w3.org/2000/svg">
              <path clip-rule="evenodd" d="M3.40717 4.46881C3.11428 4.17591 3.11428 3.70104 3.40717 3.40815C3.70006 3.11525 4.17494 3.11525 4.46783 3.40815L6.99943 5.93975L9.53095 3.40822C9.82385 3.11533 10.2987 3.11533 10.5916 3.40822C10.8845 3.70112 10.8845 4.17599 10.5916 4.46888L8.06009 7.00041L10.5916 9.53193C10.8845 9.82482 10.8845 10.2997 10.5916 10.5926C10.2987 10.8855 9.82385 10.8855 9.53095 10.5926L6.99943 8.06107L4.46783 10.5927C4.17494 10.8856 3.70006 10.8856 3.40717 10.5927C3.11428 10.2998 3.11428 9.8249 3.40717 9.53201L5.93877 7.00041L3.40717 4.46881Z"
                    fill="currentColor"
                    fill-rule="evenodd"/>
            </svg>
          </button>
      ...
      <svg :class="{ 'transform rotate-180': isOpen }" class="ml-auto" fill="none" height="20"
           viewBox="0 0 20 20"
    ...
    <transition enter-active-class="transition duration-100 ease-out"
      ...
      <div v-if="isOpen"
        ...
        <ul
          ...
          class="overflow-y-auto divide-y divide-gray-200 custom-scrollbar max-h-60 dark:divide-gray-800" role="listbox">
          <li v-for="item in props.options" :key="item.value" :aria-selected="isSelected(item)"
              :class="{ 'bg-gray-50 dark:bg-white/[0.03]': isSelected(item) }"
              class="relative flex items-center w-full px-3 py-2 border-transparent cursor-pointer first:rounded-t-lg last:rounded-b-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-800" role="option"
              @click="toggleItem(item)">
            <span class="grow">{{ item.label }}</span>
            <svg v-if="isSelected(item)" class="w-5 h-5 text-gray-400 dark:text-gray-300"
                 fill="none"
