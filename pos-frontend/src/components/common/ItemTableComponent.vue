<template>
  <div class="overflow-hidden rounded-lg border border-gray-200 shadow-md m-5">
    <table class="w-full border-collapse bg-white text-left text-sm text-gray-500">
      ...
      <tr>
        <th v-for="(header, index) in headers" :key="index" class="px-6 py-4 font-medium text-gray-900"
            scope="col">
          {{ header.text }}
        </th>
      ...
      <tr v-for="item in items" v-else :key="item.id" class="hover:bg-gray-50">
        <td v-for="(header, index) in headers" :key="index" :class="[
          ...
          <div v-if="header.value === 'name'" class="flex items-center">
            <div class="relative h-10 w-10">
              <img :alt="item.name"
                   :src="item.imagePath || 'https://i.ibb.co/3mWK7C3P/logo.png'" class="h-full w-full rounded-full object-cover object-center"/>
              <span
        ...
        <td class="px-6 py-4">
          <div class="flex justify-end gap-4">
            <a href="#" x-data="{ tooltip: 'Delete' }" @click.prevent="$emit('delete-item', item.id)">
              <svg class="h-6 w-6" fill="none" stroke="currentColor"
                   stroke-width="1.5" viewBox="0 0 24 24" x-tooltip="tooltip" xmlns="http://www.w3.org/2000/svg">
